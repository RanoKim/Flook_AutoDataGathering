...
public class MangaeRecipeCrawling {
	...
	public RecipeVO crawling(String html, String url) {
		...
		if (!userManager.isUserImageLink(userInfo[USER_IMAGE_LINK])) {
			int user = userDAO.insertUser(userInfo[USER_NAME], userInfo[USER_IMAGE_LINK],
					"flook" + (++userNo) + "@gmail.com", "0000", "female", postDate);
		}
	...
	public ArrayList<CookingVO> getCookingList(String html) {
		...
		for (Element element : e) {
			...
			imageLink.next().attr("src");imageLink.next().attr("src");imageLink.next().attr("src");
	...
	public ArrayList<IngredientVO> getIngredientInfo(String html) {
		...
		for (int i = 1; i < ingredient.length - 1; i += 2) {
			list.add(new IngredientVO(null, null, ingredient[i], 0, "M", "IUC007"));
			/*
			 * if(i%2==1) System.out.print("재료 : "+ingredient[i]); else
		...
		if (ingredientList.length > 2) { // 여기에 들어오면 하려는 요리가 2가지 이상.
			...
			for (int i = 1; i < ingredient2.length - 1; i += 2) {
				list.add(new IngredientVO(null, null, ingredient2[i], 0, "M", "IUC007"));
				/*
				 * if(i%2==1) System.out.print("재료 : "+ingredient2[i]); else
	...
	public RecipeVO getRecipeInfo(String html) {
		...
		return new RecipeVO(null, completeImage, recipeName, recipeDescription, personNumber, cookingTime,
				cookingLevel);
